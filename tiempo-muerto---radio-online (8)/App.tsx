...
const DashboardHome: React.FC<{ trackInfo: any, localData: any }> = ({ trackInfo, localData }) => {
  return (
    <div className="flex flex-col xl:flex-row gap-6 items-start animate-fade-in">
      ...
      {/* SIDEBAR IZQUIERDO: REMANSO + COLABORAR + ESTADÍSTICAS */}
      ...
      {/* CONTENIDO DERECHO: PLAYER -> INFORMACIÓN -> PUBLICIDAD -> COMUNIDAD */}
      <div className="flex-1 min-w-0 flex flex-col gap-6">
         
         {/* 1. REPRODUCTOR (Fijo arriba) */}
         <Player trackInfo={trackInfo} localData={localData} />

         {/* 2. FILA SUPERIOR: TITULARES DE HOY | MSN WEATHER */}
         ...
         {/* 3. ESPACIO PARA BOTONES DE PUBLICIDAD (AL MEDIO Y SEPARADO) */}
         {/* El margen y el relleno aseguran que no se pegue a lo de arriba ni a lo de abajo */}
         <div className="my-4 py-6 border-y border-slate-100 dark:border-slate-800">
            <div className="mb-4 flex items-center gap-2 px-2">
               <div className="h-1 w-8 bg-[#b91c1c] rounded-full"></div>
               <span className="text-[10px] font-black uppercase tracking-[0.3em] text-slate-400">Espacio Publicitario</span>
            </div>
            <AdvertisingSection slot="middle" />
         </div>

         {/* 4. FILA INFERIOR: REPORTERO VECINAL | FOGÓN DE SALUDOS */}
         <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <CitizenReporterWidget />
            <GreetingsWall />
         </div>

         {/* 5. SECCIÓN FINAL: OTROS WIDGETS */}
...
const Layout: React.FC = () => {
  ...
  useEffect(() => {
    ...
    return () => { clearInterval(trackInterval); clearInterval(weatherInterval); };
  }, []);

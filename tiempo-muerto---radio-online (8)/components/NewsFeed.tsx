  ...
  return (
    ...
    `}>
      ...
      <div className={`flex justify-between items-start ${isRow ? 'order-2 w-20 flex-col items-end' : 'mb-3'}`}>
         <span className={`text-[7px] font-black px-1.5 py-0.5 rounded-md uppercase tracking-wider shadow-sm truncate max-w-full ${item.source === 'Tiempo Muerto' ? 'bg-[#affc41] text-[#064e3b]' : 'bg-[#c1121f] text-white'}`}>
           {item.source || 'Local'}
         </span>
         {!isRow && (
...
const NewsFeed: React.FC<NewsFeedProps> = ({ categoryFilter, topic = 'general', title, mode = 'carousel' }) => {
  ...
  const fetchNewsData = async (isBackground: boolean) => {
    ...
    try {
      // El servicio ahora SIEMPRE devuelve un array (aunque sea de fallback), nunca null
      const news = await getLocalNews(topic);
      setNewsItems(news);
    } catch (error) {
      console.error("Error crítico en UI de noticias:", error);
    } finally {
      setIsLoading(false);
      if (isBackground) setTimeout(() => setIsRefreshing(false), 2000);
    }
  };
  ...
  return (
    <div className={`bg-white dark:bg-slate-900 rounded-[2.5rem] overflow-hidden shadow-xl border transition-all duration-700 h-full flex flex-col ${
      isRefreshing 
        ? 'border-[#affc41] shadow-[0_0_15px_rgba(175,252,65,0.15)]' 
        : 'border-slate-100 dark:border-slate-800'
    }`}>
      {/* Header Fijo */}
      <div className="bg-[#064e3b] p-3 flex items-center justify-between border-b border-green-700 flex-shrink-0 relative overflow-hidden">
        
        {/* INDICADOR VISUAL DE ACTUALIZACIÓN */}
        {isRefreshing && (
            <div className="absolute bottom-0 left-0 h-[2px] bg-[#affc41] w-full animate-pulse shadow-[0_0_8px_#affc41]"></div>
        )}

        <div className="flex items-center gap-3">
          <div className={`w-7 h-7 rounded-lg flex items-center justify-center transition-all duration-500 ${isRefreshing ? 'bg-[#affc41]/20 text-[#affc41]' : 'bg-white/10 text-green-400'}`}>
          ...
          <div className="flex flex-col justify-center">
            ...
            <div className="flex items-center gap-2 mt-0.5 h-3">
               <p className={`text-[7px] font-bold uppercase tracking-tighter transition-colors duration-300 ${isRefreshing ? 'text-[#affc41]' : 'text-green-400'}`}>
                 {isRefreshing ? 'ACTUALIZANDO PORTALES...' : (mode === 'carousel' ? 'Portales Locales' : 'Info Local')}
               </p>
        ...
        {mode === 'carousel' && !isLoading && newsItems.length > 0 && (

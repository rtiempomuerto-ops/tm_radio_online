...
const SUPABASE_URL = 'https://oenwcqfrohgyxuqcitxq.supabase.co'; 
const SUPABASE_ANON_KEY = 'sb_publishable_UGbs2_2laQRwUMiEaZ47kg_d0js672V'; 
...
export const subscribeToGlobalStats = (callback: (val: number) => void) => {
  return supabase.channel('stats_changes')
    .on('postgres_changes', { event: '*', schema: 'public', table: 'global_stats' }, (payload) => {
      ...
      if (newRecord && newRecord.id === 'mates_cebados') {
        callback(newRecord.counter_value);
      }
    })
    .subscribe();
};
